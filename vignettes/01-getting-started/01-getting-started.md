...
SemanticSpacetime.jl is a Julia implementation of the [Semantic
Spacetime](https://markburgess.org/spacetime.html) (SST) knowledge graph
framework, based on Mark Burgess’s Promise Theory and spacetime process
semantics. Unlike traditional knowledge graphs that store flat triples,
SST classifies every relationship along a *spacetime spectrum* — from
spatial proximity (NEAR) through causal ordering (LEADSTO) and
containment (CONTAINS) to expressive properties (EXPRESS).

This vignette walks through the basics: creating an in-memory graph,
adding nodes and edges, and querying the store.
...
The `MemoryStore` is an in-memory graph that requires no database. It is
ideal for experimentation, testing, and lightweight applications.
...
Before adding edges, we must register the *arrow types* (relationship
kinds) we plan to use. Each arrow belongs to one of the four SST types
and has a direction (`"+"` for forward, `"-"` for inverse).
...
Nodes are created with `mem_vertex!`, which takes a store, a name (text
label), and a chapter (section/topic grouping). Node creation is
*idempotent* — adding the same name and chapter twice returns the
existing node.
...
Each node has a `NodePtr` — a two-part identifier consisting of a
text-size class and an index. This bucketing by text length is a core
SST design for efficient storage.
...
Edges connect nodes with a named arrow type. The arrow name must match a
registered arrow.
...
Edges can also carry **context** (topic tags) and **weight** (numerical
strength):
...
Every node has an *incidence list* organized into 7 channels
corresponding to the signed STType spectrum (-EXPRESS to +EXPRESS).
Let’s inspect the links from our “boil water” node:
...
This was a quick introduction to SemanticSpacetime.jl. Subsequent
vignettes cover:

- **SST Types** — the spacetime type system (NEAR, LEADSTO, CONTAINS,
  EXPRESS)
- **Building Graphs** — modelling a real domain with chapters and
  contexts
- **N4L Language** — the Notes for Learning markup for declarative graph
  construction

...
how **Directed Acyclic Graphs (DAGs)** make causal assumptions explicit
and guide correct statistical adjustment.
...
framework — confounding, mediation with overcontrol, and collider bias —
then use SST graph traversal to identify backdoor paths, valid
adjustment sets, and causal versus non-causal pathways.

### Reference

> Arif, S. & MacNeil, M.A. (2023). Applying the structural causal model
> framework for observational causal inference in ecology. *Ecological
> Monographs*, 93(1), e1554.
...
```
    ...
    SemanticSpacetime.jl ecological causal inference vignette
...
```
    ...
    Domain ontology: 23 nodes, 44 links

### Annotating variables with measurement properties

In the SCM framework, variables have types (continuous, categorical) and
observability status (measured, unmeasured/latent). We use **EXPRESS**
arrows to annotate these properties.
...
```
    ...
    DAG 1 built: Temperature confounds Herbivores → Coral Cover
...
Coral Cover. We can find this using SST’s path-finding and cone search.
...
```
    ...
    Forward cone from Temperature:
      ...
      → Measured: °C, continuous [variable metadata]
...
```
    ...
    Backward cone into Coral Cover:
...
```
    ...
    Paths from Herbivores → Coral Cover: 1
      Path 1: Herbivorous Fish Biomass → Hard Coral Cover

### The adjustment set

The **backdoor criterion** says: to estimate H → C, block all backdoor
paths. The only backdoor path is H ← T → C. Conditioning on Temperature
blocks it. We represent this finding as metadata.
...
```
    ...
    DAG 2 built: Herbivores → Macroalgae → Coral Cover
...
```
    ...
    All paths Herbivores → Coral Cover: 2
      1: Herbivorous Fish Biomass → Hard Coral Cover
      2: Herbivorous Fish Biomass → Macroalgal Cover → Hard Coral Cover
...
```
    ...
    DAG 2 analysis:
      ...
      Reason: Macroalgae is on the causal pathway H → M → C
...
Structural complexity is a **collider** on the path Nutrients →
Structural Complexity ← Wave Exposure.
...
```
    ...
    DAG 3 built: Nutrients → Coral Cover with collider (Structural Complexity)

### Identifying the collider

A **collider** is a node with two or more arrows pointing *into* it from
different paths. We can detect this by examining the backward cone.
...
```
    ...
    Parents of Structural Complexity (potential collider parents):
      ← DAG 3: Collider Bias
      ← Wave Exposure
      ← Dissolved Inorganic Nutrients
    ⚠  Structural Complexity is a COLLIDER — conditioning on it is dangerous!
...
```
    ...
    DAG 3 analysis:
      ...
      ✗ Wrong:   adjust for {Structural Complexity} (collider bias!)

## 5. DAG 4 — Frontdoor Criterion: Unobserved Confounding

The most challenging scenario. Suppose **Fishing Pressure** affects both
**Herbivore Biomass** and **Coral Cover** (via reef destruction), but
fishing is **unobserved** (no reliable data). The backdoor criterion
fails. However, if we observe the **mediator** (macroalgae) and it
satisfies certain conditions, the **frontdoor criterion** allows
...
```
    ...
    DAG 4 built: Unobserved fishing confounds Herbivores → Coral

### Frontdoor analysis

The frontdoor criterion requires: 1. The mediator (Macroalgae) fully
captures the effect of Herbivores on Coral 2. No unblocked backdoor path
from Herbivores to Macroalgae 3. All backdoor paths from Macroalgae to
Coral are blocked by Herbivores
...
studies, particularly regime shifts in the Seychelles following the 1998
mass bleaching event. Let us build a concrete case study.
...
```
    ...
    12 Seychelles study sites created
...
```
    ...
    Measurements recorded for all sites
    Total graph: 122 nodes, 338 links
...
```
    ...
    Coral-dominated: Anse Royale, Baie Ternay, Cousin Island, Aldabra Atoll, Cosmoledo Atoll, Farquhar Atoll, Curieuse, Mahé West
    Algal-dominated: Anse Soleil, Praslin North, St. Pierre, Mahé East
...
```
    ...
    Orbit of 'Herbivorous Fish Biomass':
      +LEADSTO:
        Hard Coral Cover
        Macroalgal Cover
      +EXPRESS:
        Measured: % benthic cover, continuous
        Measured: kg/ha, continuous
...
```
    ...
    ────────────────────────────────────────────────────────────
      ...
      ⚡ Frontdoor identification via Macroalgae: P(C|do(H)) = Σ_M P(M|H) Σ_H' P(C|M,H') P(H')

## 9. Automated Backdoor Path Detection

We can write a general-purpose function that, given an exposure and
outcome in our graph, identifies potential confounders by finding nodes
with causal paths to *both* the exposure and the outcome.
...
```
    ...
    Potential confounders for Herbivores → Coral Cover:
      ...
      • DAG 1: Confounding [causal models]
      • Sea Surface Temperature (SST) [abiotic]
      • Fishing Pressure [human pressures]
...
```
    ...
    Potential confounders for Nutrients → Coral Cover:
...
A collider is a node where two (or more) arrows converge. We can detect
colliders by looking for nodes with multiple parents from different
causal pathways.
...
```
    ...
    Detected colliders (nodes with ≥2 causal parents):
      ...
      ⚠  Hard Coral Cover ← {Sea Surface Temperature (SST), Herbivorous Fish Biomass, Macroalgal Cover, Dissolved Inorganic Nutrients, Wave Exposure, Fishing Pressure}
...
1.  **DAG representation** — SST’s LEADSTO arrows naturally encode
    causal relationships. Unlike generic graph libraries, the four-fold
    arrow type system (NEAR, LEADSTO, CONTAINS, EXPRESS) captures the
    *semantic character* of each relationship.

2.  **Backdoor path detection** — by intersecting backward cones from
    exposure and outcome, we automatically identify potential
    confounders that the backdoor criterion requires us to adjust for.

3.  **Collider identification** — querying for nodes with multiple
    causal parents flags variables that should *not* be conditioned on.
...
The SCM framework (Arif & MacNeil, 2023) and Semantic Spacetime share a
fundamental insight: **making assumptions explicit** — whether about
